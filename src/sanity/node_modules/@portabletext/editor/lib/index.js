...
const isDeepEqual$1 = (node2, another) => {
...
function transformPath(path2, operation) {
  ...
  switch (operation.type) {
    case "insert_node": {
      ...
      (pathEquals(op, p) || pathEndsBefore(op, p) || isAncestorPath(op, p)) && (p[op.length - 1] = p[op.length - 1] + 1);
      break;
    }
    case "remove_node": {
      const {
      ...
      if (pathEquals(op, p) || isAncestorPath(op, p))
        return null;
      pathEndsBefore(op, p) && (p[op.length - 1] = p[op.length - 1] - 1);
      break;
    }
  }
  return p;
...
function createAndroidInputManager({
  ...
  const applyPendingSelection = () => {
    ...
    if (type.startsWith("delete")) {
      ...
      if (applyStringDiff(leaf$1.text, ...diffs).length === 0 && (canStoreDiff = !1), isExpandedRange(targetRange2)) {
...
}, MemoizedObjectNode = React.memo(ObjectNodeComponent, (prev, next2) => prev.objectNode === next2.objectNode && prev.renderElement === next2.renderElement && prev.isInline === next2.isInline && isElementDecorationsEqual(prev.decorations, next2.decorations));
function getTextDecorations(node2, decorations) {
  let leaves = [{
    leaf: {
      ...node2
    }
  ...
  for (const dec of decorations) {
    const {
      ...
      ...rest
    } = dec, [start2, end2] = rangeEdges(dec), next2 = [];
    let leafEnd = 0;
...
const defaultRenderLeaf = (props) => /* @__PURE__ */ jsx(DefaultLeaf, { ...props }), Leaf = (props) => {
...
}, MemoizedLeaf = React.memo(Leaf, (prev, next2) => next2.parent === prev.parent && next2.isLast === prev.isLast && next2.renderLeaf === prev.renderLeaf && next2.renderPlaceholder === prev.renderPlaceholder && next2.text === prev.text && textEquals(next2.leaf, prev.leaf) && next2.leaf[PLACEHOLDER_SYMBOL] === prev.leaf[PLACEHOLDER_SYMBOL]), DefaultLeaf = (props) => {
  const {
    attributes,
    children
  } = props;
  ...
  const {
    ...
    text
  } = props, editor = useSlateStatic(), ref = useRef(null), decorations = useDecorations(text, parentDecorations), decoratedLeaves = getTextDecorations(text, decorations), key = ReactEditor.findKey(editor, text), children = [];
...
function applyMergeNode(editor, path2, position) {
  const node2 = getNode(editor, path2, editor.schema), prevPath = previousPath(path2);
  for (const ref of editor.pathRefs) {
    const current = ref.current;
...
const insertChildren = (xs, index, ...newValues) => [...xs.slice(0, index), ...newValues, ...xs.slice(index)], replaceChildren = (xs, index, removeCount, ...newValues) => [...xs.slice(0, index), ...newValues, ...xs.slice(index + removeCount)], removeChildren = replaceChildren, modifyDescendant = (root, path2, schema, f) => {
  if (path2.length === 0)
      ...
      if (modifyChildren(editor, parentPath(path2), editor.schema, (children) => removeChildren(children, index, 1)), editor.selection) {
        ...
        for (const [point2, key] of rangePoints(selection)) {
          ...
          if (selection != null && result != null)
            ...
            let preferNext = !1;
            prev && next2 && (isSiblingPath(prev[1], path2) ? preferNext = !1 : pathEquals(next2[1], path2) ? preferNext = !0 : preferNext = commonPath(prev[1], path2).length < commonPath(next2[1], path2).length), prev && !preferNext ? selection[key] = {
              path: prev[1],
...
const getDirtyPaths = (_editor, op) => {
  switch (op.type) {
    case "insert_text":
    case "remove_text":
    case "set_node": {
...
function deleteText(editor, options = {}) {
  withoutNormalizing(editor, () => {
    ...
    if (startNonEditable) {
      ...
      beforePoint && startBlock && isAncestorPath(startBlock[1], beforePoint.path) && (start$1 = beforePoint);
    }
    if (endNonEditable) {
      const afterPoint = after(editor, end$1);
      afterPoint && endBlock && isAncestorPath(endBlock[1], afterPoint.path) && (end$1 = afterPoint);
    }
    const matches = [];
    let lastPath;
...
function insertTextBlockFragment(context, editor, block, at) {
  ...
  const parent2 = parentPath(at.path);
  let insertIndex = at.path[at.path.length - 1] + (at.offset > 0 ? 1 : 0);
  for (const child of block.children) {
    const childPath = [...parent2, insertIndex];
    editor.apply({
      type: "insert_node",
      path: childPath,
      node: child
